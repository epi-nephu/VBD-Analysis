...
rrv_raw <- DBI::dbGetQuery(con, 
                             ...
                             AND EVENT_DATE BETWEEN '", start_date, "' AND '", max_date, "'\n 
...
**Monthly cases by year from Jan 2010 - Sep 2025**
...
**Cumulative cases from Jan 2010 - Aug 2025**
...
cumsum_summer_plot <- ggplot(cumsum_summer_data) + 
  geom_line(aes(x=shifted_mth_label, y=cum_summer_count, group=factor(yr), col=factor(yr)), linewidth=1) + 
  cowplot::theme_cowplot() + 
  theme(plot.title = element_text(size=12)) + 
  labs(title="Cumulative case numbers of RRV by month in NEPHU", 
         #caption=paste0("Data period: ", start_date, " to ", max_date), 
...
Epicurve of risk factor

```{r}
tabyl(rrv_nephu, risk_factor)

rrv_nephu_risk_counts <- rrv_nephu %>% 
  group_by(epi_MY, risk_factor) %>% 
  summarise(total = n()) %>% 
  #left_join(rrv_nephu %>% group_by(epi_MY) %>% summarise(total_nephu = n()), by="epi_MY") %>% 
  replace_na(replace=list(total_nephu=0)) %>% 
  mutate(pct_nephu = round(total_nephu/total*100, 1))
...
weekly_breaks_all <- seq.Date(
  from = floor_date(min(rrv_nephu$event_date[rrv_nephu$event_date>ymd("2021-12-31")], na.rm=T),   "week", week_start = 1), # monday before first case
  to   = ceiling_date(max(rrv_nephu$event_date, na.rm=T), "week", week_start = 1), # monday after last case
  by   = "week")

monthly_breaks_all <- seq.Date(
  from = floor_date(min(rrv_nephu$event_date[rrv_nephu$event_date>ymd("2021-12-31")], na.rm=T),   "month"), # monday before first case
  to   = ceiling_date(max(rrv_nephu$event_date, na.rm=T), "month"), # monday after last case
  by   = "month")

rrv_nephu %>% 
  filter(event_date>ymd("2021-12-31")) %>% 
  ggplot() + 
  geom_histogram(aes(x=epi_MY, group=risk_factor, fill=risk_factor), col="black", breaks = monthly_breaks_all) + 
